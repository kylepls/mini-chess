...
val DIAGONAL_MASKS=longArrayOf(-0x7fbfdfeff7fbfdff,0x4020100804020100,0x2010080402010000,0x1008040201000000,0x804020100000000,0x402010000000000,
  0x201000000000000,0x100000000000000,0x0,0x80,0x8040,0x804020,0x80402010, 0x8040201008,0x804020100804,0x80402010080402)
val ANTI_DIAGONAL_MASKS=longArrayOf(0x102040810204080,0x1020408102040,0x10204081020,0x102040810,0x1020408,0x10204,0x102,0x1,0x0,0x8000000000000000UL.toLong(),
  0x4080000000000000,0x2040800000000000,0x1020408000000000, 0x810204080000000,0x408102040800000,0x204081020408000)
...
@OptIn(ExperimentalUnsignedTypes::class) class ChessBoard {
  var hmc=0; var fullMoveNumber=1; var color=0; var empty=0L; var castling=0; var enPassant=0L
  ...
  val makeMove={move:Int->
    previousCastling[hmc]=castling; previousEnPassant[hmc]=enPassant; moves[hmc]=move
  ...
  val getSingleRookAttacks={square:Int,friendlyOccupancyInv:Long->
    val fileMask=0x101010101010101 shl (square%8); val rankMask=255L shl ((square/8)*8)
    O2ON(square,fileMask,rankMask,friendlyOccupancyInv)
  }
  val getSingleBishopAttacks={square:Int,friendlyOccupancyInv:Long->
    val m1=DIAGONAL_MASKS[((square/8)-(square%8)) and 15]; val m2=ANTI_DIAGONAL_MASKS[((square/8)+(square%8)) xor 7]
    O2ON(square,m1,m2,friendlyOccupancyInv)
  }
  val O2ON={square:Int,m1:Long,m2:Long,friendlyOccupancyInv:Long->
  ...
  val getAllKnightAttacks={knights:Long->
    val l1=(knights ushr 1) and 0x7f7f7f7f7f7f7f7f; val l2=(knights ushr 2) and 0x3f3f3f3f3f3f3f3f
  ...
  val getCastleMoves={attackedSquares:Long,consumer:(Int)->Unit->
  ...
  val doublePawnPushes={color:Int,singlePawnPushes:Long->
    val s=-16*(color*2-1); val destinations=shift(singlePawnPushes,s) and empty
  ...
  val getAttackedSquares={color:Int->
    var attacks=0L
    ...
    for(i in 3..5) {
      ...
      while(pieceBitboard!=0L) {
        ...
        attacks=attacks or when(piece) { // TODO need a magic bitboard to speed this up
  ...
  val getMoves={consumer:(Int)->Unit->
    ...
    moves.addAll(getPawnMoves()) // TODO this is super expensive, remove this if possible
    moves.removeIf {
      makeMove(it)
  ...
  val captureMove={from:Int,to:Int,fromPiece:Int,toPiece:Int,color:Int->
    val fromBB=(1L shl from); val toBB=(1L shl to)
  ...
  val clear={square:Int,color:Int->
    val squareInvBB=(1L shl square).inv(); val oldPiece=this[square]
  ...
  operator fun set(square:Int,piece:Int) {
    val fromBB=1L shl square; val inv=fromBB.inv(); val color=if(piece<=6) 0 else 1
